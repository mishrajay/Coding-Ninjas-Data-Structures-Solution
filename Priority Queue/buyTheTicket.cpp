...
int buyTicket(int *arr, int n, int k){
    ...
    //push the indices in the queue
    //push the priorities in the priority queue
    for(int i = 0; i < n; i++){
        p.push(i);
        pq.push(arr[i]);
    }
    ...
    int time = 0;
    while(arr[p.front()] != pq.top() || p.front() != k){
        //if there is a person in the queue whose priority is greater than the person infront
        ...
        //if the highest priority person comes infront
        if(arr[p.front()] == pq.top()){
            pq.pop();
            p.pop();
            time++;
