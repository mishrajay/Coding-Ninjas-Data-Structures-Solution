...
#include <iostream>
using namespace std;
//do by any method, second method is commented out.
void sumOfTwoArrays(int *input1, int size1, int *input2, int size2, int *output)
{
    ...
    //run till any one loop gets exhausted
    while(i>=0 && j>=0){
    ...
    while(k >= 0) {
        while(i>=0 && j>=0){
            int	x = input1[i] + input2[j] + carry;
        }
        
        while(i<0){
            int x = input2[j] + carry;
        }
        while(j<0){
            int x = input1[i] + carry;
        }
        output[k] = x % 10;
        carry = x / 10;
        i--;
